...
window.addEventListener('DOMContentLoaded', function () {
    ...
    const adgCheck = getComputedStyle(window.document.getElementById('subscribe-to-test-important-rules-filter'), null).display == 'none';

    QUnit.test("Case 1: $important rule vs exception rule", function (assert) {
        const imageBlocked = getComputedStyle(document.querySelector("#case1 > img"), null).display === "none";
    ...
    QUnit.test("Case 2: $important rule vs exception $important rule", function (assert) {
        const imageDisplayed = getComputedStyle(document.querySelector("#case2 > img"), null).display !== "none";
